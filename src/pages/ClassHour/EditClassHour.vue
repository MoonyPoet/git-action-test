        ...
        <basic-container>
            <el-form :model="formData" :rules="rules" size="medium" ref="formData"  label-width="200px"
                ...
                <template v-if="showClassName">
                    <el-form-item label="校区" ref="campusId">
                        {{formData.campusName}}
                    </el-form-item>
                    <el-form-item label="班级名称" ref="clasId">
                        {{formData.clasName}}
                    </el-form-item>
                </template>
                <template v-else>
                ...
                <SearchBar width="100%" link="system/associative" style="width:300px;" :placeholder="$t('class.headCoach')"
                    :nameLike.sync="formData.clasHourTchrs[0].memberName"
                ...
                <el-row v-for="(item,index) in formData.clasHourTchrs" :key="item.id" v-if="index!=0" class="tch mb_10">
                    ...
                    <FormItem :label="$t('class.assistantLabel')" :isShowLabel="false">
                        <SearchBar :nameLike.sync="item.memberName" width="100%" link="system/associative" :placeholder="$t('class.assistantLabel')"
...
<script>
    ...
    export default {
        ...
        methods: {
            async find(){
               return await post("clashour/detail",{
                   clasHourId:this.$route.query.clasHourId
               })
            },
            async init() {
                 this.codeData=await getCode("campusAnt");
                let data=await this.find();
                this.formData.clasHourTchrs=data.clasHourTchrVos.map((item)=>{
                    item.memberName=item.memberName+" "+item.tel;
                    return item;
                });
                this.formData.clasHourStus=data.clasHourStus;
                this.students=data.clasHourStuVos.map((item)=>{
                    let antboxsn=this.getItemAntBoxsn(item.antsn);
                    return {
                        display:item.stuName,
                        value:item.stuId,
                        cardType:item.cardType,
                        cardUnitAmount:item.cardUnitAmount,
                        antsn:item.antsn,
                        antboxsn:antboxsn
                    }
                });
             ...
             getAntBoxsn(antboxsn,index){
                ...
                return this.antboxsn;
            },
            getItemAntBoxsn(antsn){
                let antsnItem=this.codeData.antsn.filter((item)=>{
                    return item.value===antsn;
                })
                 if(antsn&&antsnItem[0]&&antsnItem[0].antboxsn){
                  return antsnItem[0].antboxsn;
                };
            },
            submit(){
            ...
            submitForm(formName) {
                ...
                formData.clasHourId=this.$route.query.clasHourId;
                ...
                formData.clasHourTchrs.map((item)=>{
                    delete item.memberName;
                    delete item.clasHourTchrId;
                    delete item.tel;
                })
                delete formData.campusName;
                delete formData.clasName;
