    ...
    <div class="add_version_task">
        <el-form :model="formData" :rules="rules" size="medium" ref="formData" label-width="200px" label-position="left" class="demo-ruleForm">
            <el-form-item :label="$t('tchTask.trainIdLabel')" prop="trainId">
             <SelectOpt v-model="formData.trainId" @change="getTrainId" ref="trainId" :data="codeData.train" :placeholder="$t('tchTask.trainId')"></SelectOpt>
            </el-form-item>
                  ...
                  <el-row class="group" v-for="(item,index) in formData.tchTaskItems" :key="item.id">
                    ...
                    <el-row v-for="(note,nIndex) in item.tchTerms" style="padding-bottom:20px;" :key="note.id">
                      ...
                      <el-col :span="7">
                          ...
                          >
                          <SelectOpt :width="200" v-if="formData.trainId" v-model="note['trainTermId']" :data="trainTerm" :placeholder="$t('tchTask.trainTermId')"></SelectOpt>
                          </el-form-item>
                      </el-col>
                      <el-col :span="7">
                        <el-form-item
...
<script>
import { clone,getCode,validateForm} from "assets/js/reused";
