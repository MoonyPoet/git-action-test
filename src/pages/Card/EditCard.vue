...
<script>
import {clone,getCookie,validateForm,getCode,reEmphasis,isEmpty,bindData } from "assets/js/reused";
...
export default {
  ...
  methods: {
    ...
    async find(){
      let cardData=await post("card/find",{
    ...
    submitForm(formName) {
      ...
      formData.cardId=this.$route.query.cardId;
      //判断对应城市下校区是否全选传给后端为0;
      //处理成后端需要的数据
      validateForm({
