<template lang='html'>
    <Dialog ref="dialog" title="补费" @submit="submit">
         <el-form
         slot="content"
        ref="formData"
        :rules="rules"
      ...
      >
        <el-form-item
          label="欠费"
        >
          {{cardData.receivableAmount - cardData.receivedAmount}}
        </el-form-item>
        <el-form-item
          label="缴费金额"
          prop="receivedAmount"
     ...
     export default {
        ...
        methods:{
            async show(v){
                ...
                //欠费金额等于应收减去实收
                this.formData.arrears=(parseFloat(this.cardData.receivableAmount)-parseFloat(this.cardData.receivedAmount)).toFixed(2);
            },
            close(){
              this.$refs.dialog.close();
            },
            async find(v={}){
              return await post("stu/supple",v);
            },
            pushCallback(element){
                return {
            ...
            searchOpt(v){
              return {category:"member",shopId:this.shopId,keyword:v}
            },
            submit(){
                let formData = clone(this.formData);
                formData.userId=this.$route.query.userId;
                delete formData.arrears;
                validateForm({
