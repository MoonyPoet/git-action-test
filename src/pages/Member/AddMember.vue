<template>
    <div class="add_member">
        ...
        <el-form-item :label="$t('member.campusNameLabel')"  prop="campusId">
           <SelectOpt v-model="formData.campusId" ref="campusId" :data="codeData.campusId" :placeholder="$t('member.campusId')"></SelectOpt>
        </el-form-item>
        <el-row>
           <el-col :span="8">
              <el-form-item :label="$t('member.roleIdLabel')"  prop="roleId">
                <SelectOpt v-model="formData.roleId" ref="roleId" :data="codeData.roleId" :placeholder="$t('member.roleId')"></SelectOpt>
...
<script>
import {clone,getCookie,validateForm,getCode} from "assets/js/reused";
...
import {getCampusVods} from "assets/js/common";
import {
  validatePhone,
  validateCard,
  validateIdCard,
...
export default {
  ...
  methods: {
    async init(){
      this.codeData=await getCode("campus,role,_sex,_marriage_status,_political_outlook");
      this.$nextTick(()=>{
        this.$refs.campusId.selectDefaultData();
        this.$refs.roleId.selectDefaultData();
      })
    },
    showMore(){
