...
const wiierror = {
	...
	init(){
		...
		//重写open方法,记录请求的url
		XMLHttpRequest.prototype.open = function(method, url, boolen=true) {
	...
	sendReport(data) {
		...
		var reqData = Object.assign({}, this.options, data, {
			timespan: dateFormat("yyyy-MM-dd hh:mm:ss",new Date()),
			url: location.href
		})
		console.log(reqData)
		if(reqData.msg==="script is load error"){
			Vue.prototype.$alert("客户端有部分更新,刷新页面即可", {
