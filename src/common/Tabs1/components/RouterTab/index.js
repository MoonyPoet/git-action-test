...
import {getLocal} from "assets/js/reused";
...
export default {
  ...
  methods: {
    ...
    getTabData(){
      ...
      // console.log(tabs) 
      //如果没有index 则存入index
      if(!this.$store.state.options.some((item)=>{item.route==='index'})){
      ...
      //   route.push(this.getRouteTab(element.route))
      // })
      console.log(route)
      return route;
    },
    setRoute(){
      if (getLocal("options") == undefined) {
        this.$store.commit("setOptions", [{ route: "/index" }]);
    ...
    getRouteTab (route, matchRoutes = this.matchRoutes(route)) {
      ...
      let {icon, tips} = matchRoutes.pageRoute.meta;
      let title=matchRoutes.pageRoute.name;
