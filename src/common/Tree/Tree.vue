...
<script>
    ...
    function setIndex(list){
      let obj={};
      function set(list,path){
        if(list&&list.length>0){
          list.map((element)=>{
            let path1=element.name+"-"+element.id+(path?'.'+path:"");
            obj[path1]=element;
            if(element.list){
              set(element.list,path1)
            }
          })
    ...
    //查询
    function find(element,source){
      if(element){
        let arr=Object.keys(source);
        let keyArr=[]
        arr.map((item)=>{
          if(item.indexOf(element.name+'-'+element.id)>-1){
            keyArr.push(item)
          }
        })
        keyArr=keyArr.sort();
        ...
        let strPath=path1.join(".");
        return source[strPath];
      }
    }
    function findParentCheck(element,treeObj){
      let parent= find(element,treeObj);
      if(parent){
        parent.checked=true;
        findParentCheck(parent,treeObj)
      }else{
        return
      }
    }
            ...
            treeItem:{
              ...
              methods:{
                ...
                change(check,element,root){
                  this.$nextTick(()=>{
                    findParentCheck(element,treeObj)
                    if(!isEmpty(element.list)){
                      if(check){ 
